...
def process_manifest_row(row, cutoff):
    ...
    if os.path.exists(f"{args.pdb_id}_bound_entity_contacts.tsv"):
        ...
        contacts_poly_merged_filtered[["domain_accession", "bgn", "end", "contact", "auth_seq_range", "end_auth_asym_id", "xref_db"]].to_csv(f"{args.pdb_id}_pymol_contacts.csv.gz", compression = "gzip")
        ...
        bound_entity_info_arp_exploded_merged_aggregated = bound_entity_info_arp_exploded_merged_aggregated.loc[bound_entity_info_arp_exploded_merged_aggregated.domain_residue_interactions.str.len() >= args.domain_contact_cutoff]
