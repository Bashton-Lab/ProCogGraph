...
def main():
    ...
    for index, row in manifest.iterrows():
        ...
        if len(branched_seq_info_merged) > 0 or len(nonpoly_seq_info_filtered) > 0:
            ...
            logs.append(log)
            final_manifest.append(f"{args.pdb_id},{args.assembly_id},{args.cif},{args.bio_h},{args.sifts_xml},{cwd}/{args.pdb_id}_arpeggio.csv,{cwd}/{args.pdb_id}_bound_entity_info.pkl")
        else:
            log = f"{pdb_id},122,No bound entities found in cif file"
            logs.append(log)
            continue
    print(logs)
    with open("logs.txt", mode='w', newline='') as file:
        for line in logs:
            file.write("%s\n" % line)
    with open("new_manifest.txt", mode = "w", newline = '') as file:
